...
def create_master_skill(integration_name, base_path):
    ...
    if not validate_name(integration_name):
        print(f"[ERROR] Invalid name: {integration_name}")
    ...
    print(f"   [OK] Created folder structure")
...
""")
    print(f"   [OK] Created tests/README.md")
...
def test_discover_script_exists():
    ...
    assert os.path.exists(script), f"Script not found: {{script}}"
    print("[OK] Discover script exists")
...
def test_references_exist():
    ...
    for ref in required:
        ...
        print(f"[OK] {{ref}} exists")


def main():
    print(f"\\n[TEST] Testing {integration_name}-master skill...\\n")
    ...
    print(f"\\n[RESULT] Results: {{passed}} passed, {{failed}} failed")
...
if __name__ == '__main__':
    main()
''')
    print(f"   [OK] Created tests/run_tests.py")

    print(f"\n[OK] Created: {skill_path}")
    print(f"\n[>] Next steps:")
