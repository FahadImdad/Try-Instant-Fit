...
USER_SKILLS_DIR = PROJECT_ROOT / "03-skills"  # Default: user integrations
SYSTEM_INTEGRATIONS_DIR = PROJECT_ROOT / "00-system" / "skills" / "integrations"  # --system flag
...
def create_connect_skill(config: dict, output_dir: Path, skills_base_path: str):
    """Create the connect meta-skill

    Args:
        config: Integration configuration
        output_dir: Directory to create skill in
        skills_base_path: Base path for scripts (e.g., "03-skills" or "00-system/skills/integrations")
    """
...
4. Copy the key'''
    ...
    # Workflows - use dynamic base path
    workflows = ""
...
```bash
python {skills_base_path}/{config['service_slug']}/{config['service_slug']}-master/scripts/{ep['slug']}.py --json
```
...
def scaffold_integration(config: dict, system_integration: bool = False):
    """Main scaffolding function

    Args:
        config: Integration configuration dict
        system_integration: If True, create in 00-system/skills/integrations/ (internal dev only)
                           If False (default), create in 03-skills/ (user integrations)
    """
    base_dir = SYSTEM_INTEGRATIONS_DIR if system_integration else USER_SKILLS_DIR
    skills_base_path = "00-system/skills/integrations" if system_integration else "03-skills"
    target_desc = "system integration" if system_integration else "user skill"

    print(f"\nScaffolding {config['service_name']} integration as {target_desc}...")
    print("=" * 50)

    output_dir = base_dir / config['service_slug']
    ...
    create_connect_skill(config, output_dir, skills_base_path)
...
def main():
    ...
    parser.add_argument(
        "--system",
        action="store_true",
        help="Create in 00-system/skills/integrations/ (internal dev only). Default: 03-skills/"
    )
    ...
    scaffold_integration(config, system_integration=args.system)
