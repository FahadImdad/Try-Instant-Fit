...
def list_all_datasets(limit: int = 100, max_pages: int = None) -> list:
    """Fetch all datasets across all pages.
    ...
    while True:
        result = list_datasets(limit=limit, page=page)
        ...
        print(f"Fetched page {page}/{total_pages} ({len(data)} datasets)", file=sys.stderr)
    ...
    print(f"Total: {len(all_datasets)} datasets", file=sys.stderr)
    return all_datasets
...
def main():
    ...
    try:
        if args.all:
            result = list_all_datasets(limit=args.limit, max_pages=args.max_pages)
            output = {"data": result, "meta": {"totalItems": len(result)}}
        else:
            output = list_datasets(limit=args.limit, page=args.page)
