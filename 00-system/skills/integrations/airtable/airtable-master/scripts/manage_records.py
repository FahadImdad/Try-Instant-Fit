...
def _resolve_base_id(base_ref, token_name=None, verbose=False):
    ...
    print(f"[ERROR] Could not find base: {base_ref}")
    sys.exit(1)
        ...
        if response.status_code == 200:
            ...
            if verbose:
                for rec in created_records:
                    print(f"      [OK] Created {rec['id']}")
        else:
            error = response.json().get('error', {})
            print(f"   [ERROR] Create failed: {error.get('message', response.status_code)}")
            if verbose:
        ...
        if response.status_code == 200:
            ...
            if verbose:
                for rec in updated_records:
                    print(f"      [OK] Updated {rec['id']}")
        else:
            error = response.json().get('error', {})
            print(f"   [ERROR] Update failed: {error.get('message', response.status_code)}")
            if verbose:
                print(f"      Response: {response.text[:500]}")
        ...
        if response.status_code == 200:
            ...
            if verbose:
                for rec in deleted_records:
                    print(f"      [OK] Deleted {rec['id']}")
        else:
            error = response.json().get('error', {})
            print(f"   [ERROR] Delete failed: {error.get('message', response.status_code)}")
            if verbose:
                print(f"      Response: {response.text[:500]}")
...
def main():
    ...
    if args.action == 'create':
        if not records_data:
            print("[ERROR] --data or --file required for create")
            sys.exit(1)
        ...
        if not args.record and not args.file:
            print("[ERROR] --record or --file required for update")
            sys.exit(1)

        if args.record:
            if not args.data:
                print("[ERROR] --data required when using --record")
                sys.exit(1)
        ...
        if not args.record and not args.file:
            print("[ERROR] --record or --file required for delete")
            sys.exit(1)
        ...
        if not args.json:
            print(f"\n[DEL]  Deleting {len(record_ids)} record(s) from {args.table}...")
    ...
    if args.json:
        ...
        print(f"\n[OK] {args.action.title()}d {len(result)} record(s)")
