...
def check_env_file(verbose=False):
    ...
    if not env_path.exists():
        ...
        return False, "[ERROR] .env file not found", result
    ...
    if not api_key:
        result['missing'] = ['AIRTABLE_API_KEY']
        return False, "[ERROR] AIRTABLE_API_KEY not set in .env", result
    ...
    return True, "[OK] .env file configured", result
...
def check_user_config(verbose=False):
    ...
    if config is None:
        return 'partial', "[!]  user-config.yaml not found or invalid", result
    ...
    if user_id or default_base:
        msg = f"[OK] User config present"
        if default_base:
            msg += f" (default base: {default_base[:15]}...)"
        return True, msg, result
    else:
        return 'partial', "[INFO]  No Airtable settings in user-config.yaml (optional)", result
...
def check_api_connection(verbose=False):
    ...
    if not api_key:
        ...
        return False, "[ERROR] No API key available", result
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return True, f"[OK] API connection successful ({result['user_email']})", result
        elif response.status_code == 401:
            result['error'] = "Invalid API key (401 Unauthorized)"
            return False, "[ERROR] API key invalid (401 Unauthorized)", result
        elif response.status_code == 403:
            result['error'] = "Insufficient permissions (403 Forbidden)"
            return False, "[ERROR] API key lacks permissions (403 Forbidden)", result
        else:
            result['error'] = f"API returned status {response.status_code}"
            return False, f"[ERROR] API returned status {response.status_code}", result

    except requests.exceptions.Timeout:
        result['error'] = "Connection timeout"
        return False, "[ERROR] API connection timed out", result
    except requests.exceptions.RequestException as e:
        result['error'] = str(e)
        return False, f"[ERROR] API connection failed: {e}", result
...
def check_base_access(verbose=False):
    ...
    if not api_key:
        ...
        return False, "[ERROR] No API key", result
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if bases:
                ...
                return True, f"[OK] Base access confirmed ({len(bases)} base(s))", result
            else:
                return 'partial', "[!]  No bases accessible", result
        else:
            result['error'] = f"Could not list bases: {response.status_code}"
            return False, f"[ERROR] Could not list bases: {response.status_code}", result

    except requests.exceptions.RequestException as e:
        result['error'] = str(e)
        return False, f"[ERROR] Base check failed: {e}", result
...
def main():
    ...
    if env_result.get('api_key_format_warning'):
        print(f"      [!]  {env_result['api_key_format_warning']}")
    ...
    if api_ok:
        ...
        print("      [SKIP]  Skipped (no API connection)")

    print()

    # Determine exit code and summary
    if env_ok and api_ok and base_ok == True:
        print("[OK] Airtable configuration complete!")
        print()
        print("You're ready to use Airtable skills.")
        sys.exit(0)
    elif env_ok and api_ok:
        print("[!]  Airtable partially configured")
        print()
