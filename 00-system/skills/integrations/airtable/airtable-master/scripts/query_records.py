    ...
    while True:
        ...
        try:
            ...
            if response.status_code == 404:
                print(f"[ERROR] Table not found: {table_ref}")
                print("   Check table name or use table ID (tblXXX)")
                sys.exit(1)

            if response.status_code == 422:
                error = response.json().get('error', {})
                print(f"[ERROR] Invalid request: {error.get('message', 'Unknown error')}")
                sys.exit(1)
...
def main():
    ...
    # Query records
    if not args.json:
        print(f"\n[SEARCH] Querying {args.table} in {args.base}...")
    ...
    if args.json:
        ...
        print(f"\n[STATS] Found {len(records)} record(s)\n")
