    ...
    try:
        if args.command == "list":
            ...
            if attendees:
                ...
                print("[!]  Attendees will receive a calendar invite!")
            if reminders:
            ...
            if not args.yes:
                ...
                if confirm not in ['yes', 'y']:
                    print("[ERROR] Event creation cancelled.")
                    sys.exit(0)
            ...
            if args.json:
                ...
                if result.get('attendees_notified'):
                    print("[EMAIL] Invites sent to attendees")

        elif args.command == "quick-add":
            # Show preview
            print("\n" + "=" * 50)
            print("[DATE] QUICK ADD EVENT")
            print("=" * 50)
            ...
            if current.get('attendees'):
                print("[!]  Attendees will be notified of changes!")
            print("=" * 50)
            ...
            if args.json:
                ...
                print(f"Link: {result['htmlLink']}")
                if result.get('attendees_notified'):
                    print("[EMAIL] Attendees notified of changes")
            ...
            if current.get('attendees'):
                ...
                if not args.no_notify:
                    print("[!]  Attendees will be notified of cancellation!")
            print("=" * 50)

            if not args.yes:
                confirm = input("\n[!]  Delete this event? (yes/no): ").strip().lower()
                if confirm not in ['yes', 'y']:
            ...
            print(f"\n[OK] Event deleted: {result['summary']}")
            if result.get('attendees_notified'):
                print("[EMAIL] Attendees notified of cancellation")
            ...
            print("\n" + "=" * 50)
            print("[USERS] ADD ATTENDEES")
            print("=" * 50)
            print(f"Event: {current['summary']}")
            print(f"Adding: {', '.join(attendees)}")
            print("[!]  New attendees will receive calendar invites!")
            print("=" * 50)

            if not args.yes:
                confirm = input("\n[!]  Add these attendees? (yes/no): ").strip().lower()
                if confirm not in ['yes', 'y']:
                    print("[ERROR] Cancelled.")
                    sys.exit(0)

            result = add_attendees(args.event_id, attendees, args.calendar)
            print(f"\n[OK] Added {len(attendees)} attendee(s)")
            ...
            if not args.yes:
                confirm = input("\n[!]  Remove these attendees? (yes/no): ").strip().lower()
                if confirm not in ['yes', 'y']:
                    print("[ERROR] Cancelled.")
                    sys.exit(0)

            result = remove_attendees(args.event_id, attendees, args.calendar)
            print(f"\n[OK] Removed {len(attendees)} attendee(s)")
