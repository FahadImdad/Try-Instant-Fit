...
class SlackTestSuite:
    ...
    def run_test(self, name, category, test_func):
        ...
        # Log progress
        icon = "[OK]" if result.status == "passed" else "[SKIP]" if result.status == "skipped" else "[ERROR]"
    ...
    def setup(self):
        ...
        for user in users:
            ...
            if (TEST_USER.lower() in [username, display_name] or
                ...
                self.log(f"  [OK] Found @{user.get('name')} ({user.get('real_name')}) - {self.test_user_id}")
                break

        if not self.test_user_id:
            self.log(f"  [ERROR] Could not find user @{TEST_USER}")
            return False
        ...
        self.log(f"  [OK] DM channel: {self.test_dm_channel}")
    ...
    def test_update_message(self, result):
        ...
        response = self.client.post('chat.update', {
            ...
            'text': "[TEST] *Nexus Slack Integration Test*\n\n✏ This message was updated at " + datetime.now().strftime('%H:%M:%S')
        })
        if response.get('ok'):
    ...
    def test_send_channel_message(self, result):
        ...
        response = self.client.post('chat.postMessage', {
            'channel': self.test_channel_id,
            'text': "[TEST] Test message in private test channel"
        })
        if response.get('ts'):
    ...
    def test_upload_file(self, result):
        ...
        complete_response = self.client.post('files.completeUploadExternal', {
            ...
            'initial_comment': '[DIR] Test file upload (will be deleted)'
        })
    ...
    def test_schedule_message(self, result):
        ...
        response = self.client.post('chat.scheduleMessage', {
            'channel': self.test_dm_channel,
            'text': '[TEST] Scheduled test message (should be cancelled before sending)',
            'post_at': post_at
        })
    ...
    def cleanup(self):
        ...
        if self.test_message_ts:
            try:
                ...
                self.log(f"  [OK] Deleted test message")
            except SlackAPIError as e:
                self.log(f"  [!] Could not delete test message: {e.error_code}")

        # Delete test file if still exists
        if self.test_file_id:
            try:
                self.client.post('files.delete', {'file': self.test_file_id})
                self.log(f"  [OK] Deleted test file")
            except SlackAPIError as e:
                self.log(f"  [!] Could not delete test file: {e.error_code}")

        # Delete test reminder if still exists
        if self.test_reminder_id:
            try:
                self.client.post('reminders.delete', {'reminder': self.test_reminder_id})
                self.log(f"  [OK] Deleted test reminder")
            except SlackAPIError as e:
                self.log(f"  [!] Could not delete test reminder: {e.error_code}")
        ...
        if self.scheduled_message_id:
            try:
                ...
                self.log(f"  [OK] Cancelled scheduled message")
            except SlackAPIError as e:
                self.log(f"  [!] Could not cancel scheduled message: {e.error_code}")
        ...
        if self.test_channel_id:
            try:
                ...
                self.log(f"  [OK] Archived test channel")
            except SlackAPIError as e:
                self.log(f"  [!] Could not archive test channel: {e.error_code}")

        # Send cleanup complete message
        try:
            self.client.post('chat.postMessage', {
                'channel': self.test_dm_channel,
                'text': "[OK] *Nexus Slack Integration Test Complete*\n\nAll test artifacts have been cleaned up."
            })
        except:
            pass
    ...
    def print_summary(self):
        ...
        if self.json_output:
            ...
            self.log("=" * 60)
            self.log(f"\n  Total:   {total}")
            self.log(f"  [OK] Passed:  {passed}")
            self.log(f"  [ERROR] Failed:  {failed}")
            self.log(f"  [SKIP] Skipped: {skipped}")
            self.log("")
